...
ストアへの申請時に、以下のテキストをコピー＆ペーストしてご利用ください。
...
### 短い説明
Webページ上の入力欄で動作する、SKK風の日本語入力拡張機能です。

### 詳細な説明
ChromeのWebページ内にある入力欄（input, textarea）で、SKKのような操作感で日本語入力を可能にする拡張機能です。
OSにIMEをインストールできない環境や、ブラウザ内だけで完結させたい場合に最適です。
...
### Summary
An SKK-like Japanese input extension that works within web page inputs.

### Detailed Description
This extension enables SKK-style Japanese input directly within input fields (input, textarea) on web pages.
Ideal for environments where you cannot install an OS-level IME or prefer to keep your input environment contained within the browser.
...
- Automatically disabled in password fields and other sensitive input types.
